...
const app = express();
const stripe = require("stripe")("sk_test_51OFVfqDqdT1eaxGhxMfh2d7CKTogF7tpk0syj6jTysiIsGD6kY3zvcAmlTUi5N4Pc4EpYjKobahrBWKbQA69a7sq00faNyVnbZ")
...
async function run() {
  try {
    ...
    const paymentCollection = client.db("assertManagement").collection("payments");
    ...
    const customAssetCollection = client.db("assertManagement").collection("custom-assets");
    const teamCollection = client.db("assertManagement").collection("teams");
    const reqAssetCollection = client.db("assertManagement").collection("req-assets");
    ...
    app.get("/req-assets", async (req, res) => {
      const email = req.query.email;
      const query = { userEmail: email };
    ...
    app.get("/req-assets", async (req, res) => {
      const { email, searchQuery, availabilityFilter, assetTypeFilter, requestStatusFilter } = req.query;
      ...
      try {
        ...
        if (availabilityFilter !== 'all') {
          query.isPending = availabilityFilter === 'pending';
        }
        ...
        console.error("Error fetching req-assets:", error);
    ...
    app.get("/req-assets", async (req, res) => {
      const { email, searchQuery, availabilityFilter, assetTypeFilter } = req.query;
